...
const buildXlsxBuffer = (headers: string[], rows: ExportRow[]) => {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, 'Maintenance Cost');
...
export const handler = async (event: {
  ...
  const prefix = process.env.BUCKET_PREFIX ?? 'maintenance/';
  ...
  try {
    ...
    const fileName = filtered
      ? `maintenance-billing-${monthId}-filtered-${timestamp}.xlsx`
      : `maintenance-billing-${monthId}-${timestamp}.xlsx`;
    ...
    console.error('ExportMaintenanceBilling failed', error);
    return jsonResponse(500, 'Failed to export maintenance billing.');
  }
};
